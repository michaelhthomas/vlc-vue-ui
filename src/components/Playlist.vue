<template>
  <v-treeview open-all :items="playlist.data.children" :active="activeList">
    <template #prepend="{ item, open }">
      <v-icon v-if="item.type == 'node'">
        {{ open ? "mdi-folder-open" : "mdi-folder" }}
      </v-icon>
      <v-icon v-else> mdi-filmstrip-box </v-icon>
    </template>
    <template #label="{ item }">
      <a v-if="item.type == 'leaf'" @click="activate(item.id)">{{
        item.name
      }}</a>
...
export default {
  ...
  computed: {
    ...
    activeList: function () {
      let filterResult = this.recursiveFilter(
        this.playlist.data,
        (e) => e.current == "current"
      );
